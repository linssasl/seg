...
class STLProcessingApp:
    ...
    def process_file(self):
        ...
        if not self.save_directory:
            messagebox.showwarning("警告", "请先选择生成文件存储位置！")
            return
        ...
        try:
            ...
            above, below = split_model(model, center, long_axis)

            upper_path = os.path.join(self.save_directory, "upper_model.stl")
            lower_path = os.path.join(self.save_directory, "lower_model.stl")

            save_stl(above, upper_path)
            save_stl(below, lower_path)

            plot_heatmap_on_section(above.reshape(-1, 3), center, long_axis, self.save_directory, "upper")
            plot_heatmap_on_section(below.reshape(-1, 3), center, long_axis, self.save_directory, "lower")
